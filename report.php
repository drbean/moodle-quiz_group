...
class quiz_group_report extends quiz_attempts_report {
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        foreach ( $allgroups as $currentgroup => $groupdata )
        {
            ...
            if ($currentgroup == self::NO_GROUPS_ALLOWED) {
                $alldata[$currentgroup] = array($currentgroup, $empty, $empty, $empty);
            }

            $studentsjoins = get_enrolled_with_capabilities_join($this->context);

            if (empty($currentgroup)) {
                $alldata[$currentgroup] = array($currentgroup, $studentsjoins, $empty, $studentsjoins);
            }
    ...
    public function display($quiz, $cm, $course) {
        ...
        $allgroupdata = $this->init('group', 'quiz_group_settings_form',
                $quiz, $cm, $course);
