 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_group_table extends quiz_attempts_report_table {
     ...
     * @param quiz_group_options $options
    ...
    public function __construct($quiz, $context, $qmsubselect,
            quiz_group_options $options, \core\dml\sql_join $groupstudentsjoins,
    ...
    protected function submit_buttons() {
        if (has_capability('mod/quiz:regrade', $this->context)) {
            echo '<input type="submit" class="btn btn-secondary m-r-1" name="regrade" value="' .
                    get_string('regradeselected', 'quiz_group') . '"/>';
        }
     ...
     * mod/quiz/report/group/report.php to see what fields are present,
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'quiz_group');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'quiz_group');
        }
    }
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('quiz_group_regrades',
