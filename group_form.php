...
/**
 * This file defines the setting form for the quiz group report.
 *
 * @package   quiz_group
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_group_settings_form extends mod_quiz_attempts_report_form {

    protected function other_preference_fields(MoodleQuickForm $mform) {
        $mform->addGroup(array(
            $mform->createElement('advcheckbox', 'qtext', '',
                get_string('questiontext', 'quiz_group')),
            $mform->createElement('advcheckbox', 'resp', '',
                get_string('response', 'quiz_group')),
            $mform->createElement('advcheckbox', 'right', '',
                get_string('rightanswer', 'quiz_group')),
        ), 'coloptions', get_string('showthe', 'quiz_group'), array(' '), false);
